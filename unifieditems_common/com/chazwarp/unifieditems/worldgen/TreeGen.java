...
public class TreeGen extends WorldGenerator
{
    ...
    public TreeGen(boolean notify, int treeHeight, int treeRange, int woodMeta, int leavesMeta)
    {
        super(notify);
    ...
    public boolean generate (World world, Random random, int xPos, int yPos, int zPos)
    {
        ...
        if (yPos >= 1 && yPos + height + 1 <= 256)
        {
            ...
            boolean isSoil = (soil != null && soil.canSustainPlant(world, xPos, yPos - 1, zPos, ForgeDirection.UP, BlockInfo.rareSapling));
        ...
        for (int yPos = 0; yPos < treeHeight + 1; yPos++)
        {
            ...
            for (int xPos = range; xPos <= range; xPos++)
            {
                for (int zPos = range; zPos <= range; zPos++)
                {
                    ...
                    if (block != null && blockID != BlockInfo.RUBBER_SAPLING_ID && !block.isLeaves(world, x + xPos, y + yPos, z + zPos))
                        return false;
                }
            }
        }
            ...
            for (int x = xPos - i2; x <= xPos + i2; ++x)
            {
                ...
                for (int z = zPos - i2; z <= zPos + i2; ++z)
                {
                    ...
                    if (Math.abs(k2) != i2 || Math.abs(i3) != i2 || random.nextInt(2) != 0 && k1 != 0)
                    {
                        ...
                        {
                            this.setBlockAndMetadata(world, x, y, z, BlockInfo.RUBBER_LEAVES_ID, this.metaLeaves);
                        }
                    }
                }
            }
    ...
    void placeTrunk (World world, int xPos, int yPos, int zPos, int height)
    {
        for (int localHeight = 0; localHeight < height; ++localHeight)
        {
            ...
            {
                this.setBlockAndMetadata(world, xPos, yPos + localHeight, zPos, BlockInfo.RUBBER_WOOD_ID, this.metaWood);
            }
        }
    }
